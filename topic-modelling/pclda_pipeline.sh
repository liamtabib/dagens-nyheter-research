...
python3 topic-modelling/scripts/generate_inputs.py

echo Generating input.txt and documents.zip from topic-modelling/scripts/generate_inputs.py 

[ -s topic-modelling/input.txt ] && echo "input.txt exists and is not empty" || \
{ echo "input.txt from generate_inputs.py does not exist or is empty."; exit 1; }

# preprocess input.txt
python3 topic-modelling/scripts/preprocess.py

echo Generating input_clean.txt from topic-modelling/scripts/preprocess.py 

[ -s topic-modelling/input_clean.txt ]  && echo "input_clean.txt exists and is not empty" || \
{ echo "input_clean.txt from preprocess.py does not exist or is empty."; exit 1; }

rm topic-modelling/input.txt && echo "input.txt deleted" 
...
cp topic-modelling/dn_config.cfg "$BASE_DIR/PartiallyCollapsedLDA"
cp topic-modelling/input_clean.txt "$BASE_DIR/PartiallyCollapsedLDA"
...
python3 topic-modelling/scripts/pick_z_files.py --path_pclda_runs="$BASE_DIR/PartiallyCollapsedLDA/Runs"

# run humlabs pipeline on z_files
python3 "$BASE_DIR//PCLDA-pipeline/pclda_pipeline/convert.py" topic-modelling/z_files/*.csv --target-folder=topic-modelling/westac_hub_files
